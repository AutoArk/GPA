...
DEFAULT_HF_CACHE_DIR = PROJECT_ROOT / ".cache" / "hf_datasets"
UPSTREAM_SAMPLE_DATA = WORKSPACE_ROOT.parent / "GPA" / "scripts" / "train" / "merged_shuffled_train.jsonl"
...
def build_default_path_report() -> dict[str, str]:
    ...
    return {
        ...
        "hf_cache_default": str(DEFAULT_HF_CACHE_DIR),
        "upstream_sample_dataset": str(UPSTREAM_SAMPLE_DATA) if UPSTREAM_SAMPLE_DATA.exists() else "",
    }
...
def resolve_model_dir(explicit: str | None = None) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Checked '{path}'. Provide --model_name_or_path or set {MODEL_ENV_VAR}."
        )
    return path
...
def resolve_audio_tokenizer_dir(explicit: str | None = None) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Checked '{path}'. Provide --audio_tokenizer_path or set {TOKENIZER_ENV_VAR}."
        )
    return path