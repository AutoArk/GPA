...
def write_manifest(model_dir: Path) -> None:
    payload = {
        ...
        "llm_default": "llm_kv_cpu_fp32_int4.onnx",
...
def main() -> None:
    ...
    fp_llm = build_dir / "llm_kv_fp32.onnx"
    ...
    run([sys.executable, str(runtime_root / "scripts" / "export_ark_llm_kv.py"), "--model-dir", str(args.ark_model_dir), "--output-path", str(fp_llm), "--max-total-len", str(args.max_total_len)])
