...
class SparkTokenizer:
    def __init__(
            self,
            model_path: str,
            device: Union[Literal["cpu", "cuda", "mps"], str] = "cuda",
        ...
        # BiCodec model
        self.model = BiCodec.load_from_checkpoint(str(self.model_dir)).to(self.device)
    ...
    def _to_ndarray(self, audio_input: Union[str, Path, torch.Tensor]) -> np.ndarray:
        """
        Convert either a file path or a tensor input to a numpy waveform.
        """
        if isinstance(audio_input, (str, Path)):
            # For file paths, reuse the existing load_audio helper.
            wav = load_audio(
                str(audio_input),
            ...
            # For tensors, normalize to a mono float waveform.
            ...
            # The tensor input is assumed to already use the target sample rate.
            wav = wav.numpy()

            # Optionally normalize amplitude when the tensor is not normalized yet.
    ...
    def get_ref_clip(self, wav: np.ndarray) -> np.ndarray:
        """Extract a fixed-length reference clip."""
        ref_segment_length = (
    ...
    def process_audio(self, audio_input: Union[str, torch.Tensor], ref_audio_input: Union[str, torch.Tensor] = None) -> Tuple[np.ndarray, torch.Tensor]:
        """
        Prepare the main audio and the reference audio clip.
        """
    ...
    def extract_wav2vec2_features(self, wavs: torch.Tensor) -> torch.Tensor:
        """Extract wav2vec2 features for a batch of waveforms."""
    ...
    def tokenize(self, audios: List[Union[str, torch.Tensor]]):
        """
        Tokenize a list of audio file paths or tensors.
        """
        batch_wavs = []
        batch_ref_wavs = []
...
if __name__ == "__main__":
    # Configure your local model directory.
    MODEL_DIR = "/data/yumu/model/ark_tts_v1"
    
    # Initialize the tokenizer.
    # This example requires a real local model directory.
    tokenizer = SparkTokenizer(model_path=MODEL_DIR, device="cuda" if torch.cuda.is_available() else "cpu")
    
    # Prepare test inputs: one file path and one tensor waveform.
    dummy_wav_path = "samples/sample.mp3"
    # Build a 16 kHz waveform tensor from the same file.
    import torchaudio
    dummy_tensor, sr = torchaudio.load(dummy_wav_path)

    # 1. Test file-path input.
    ...
    # 3. Test batched tensor input.
    print("Testing mixed input...")
    # For a simple smoke test, pass the same tensor twice.
